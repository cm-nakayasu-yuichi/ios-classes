...
extension Date {
    ...
    ///   - minute: 分
    ///   - second: 秒
    mutating func fix(year: Int? = nil, month: Int? = nil, day: Int? = nil, hour: Int? = nil, minute: Int? = nil, second: Int? = nil) {
        if let value = year   { self.year   = value }
        if let value = month  { self.month  = value }
        if let value = day    { self.day    = value }
        if let value = hour   { self.hour   = value }
...
extension Date {
    ...
    ///   - minute: 分
    ///   - second: 秒
    mutating func add(year: Int? = nil, month: Int? = nil, day: Int? = nil, hour: Int? = nil, minute: Int? = nil, second: Int? = nil) {
        if let value = year   { self.year   += value }
        if let value = month  { self.month  += value }
        if let value = day    { self.day    += value }
        if let value = hour   { self.hour   += value }
...
extension Date {
    ...
    ///   - options: オプション
    /// - Returns: 日付オブジェクト
    static func dateFromISO8601String(_ string: String, options: ISO8601DateFormatter.Options? = nil) -> Date? {
    ...
    func iso8601String(options: ISO8601DateFormatter.Options? = nil) -> String {
...
extension Date {
    ...
    /// 0分0秒に揃える
    mutating func setOclock() -> Date {
        fix(minute: 0, second: 0)
        return self
    }
    
    /// 0時0分0秒に揃える
    mutating func setZeroclock() -> Date {
        fix(hour: 0, minute: 0, second: 0)
        return self
    }
    
    /// 日の先頭時刻(0時0分0秒)に揃える
    mutating func setFirstOfDay() -> Date {
        fix(hour: 0, minute: 0, second: 0)
        return self
    }
    
    /// 日の最終時刻(23時59分59秒)に揃える
    mutating func setLastOfDay() -> Date {
        fix(hour: 23, minute: 59, second: 59)
        return self
    }
    
    /// 0時0分0秒に設定された現在月の月初に揃える
    mutating func setFirstDayOfMonth() -> Date {
        fix(hour: 23, minute: 59, second: 59)
        return self
    }
    
    /// 0時0分0秒に設定された現在月の月末に揃える
    mutating func setLastDayOfMonth() -> Date {
        fix(hour: 0, minute: 0, second: 0)
        return self
    }
}
...
extension Date.Week {
    ...
    static func items(startWeek: Date.Week = .sunday) -> [Date.Week] {
...
extension Array where Element == Date {
    ...
    /// - Parameter format: フォーマット
    /// - Returns: 日付文字列の配列
    func strings(format: String = "yyyy-MM-dd HH:mm:ss") -> [String] {
